...
const ADMIN_USER_USERNAME = 'jessieinberg'; // Define the special admin username

const Navbar: React.FC<NavbarProps> = ({ isAdmin, onAdminLogout }) => {
  const [theme, toggleTheme] = useTheme();
  const navigate = useNavigate();
  const location = useLocation();
  const { currentUser, logout: userLogout, isLoading: authIsLoading } = useAuth();
  const [unreadMessages, setUnreadMessages] = useState(0);

  useEffect(() => {
    const fetchUnreadCount = async () => {
      if (!isAdmin && currentUser && !authIsLoading) {
        const count = await chatService.getUnreadMessagesCountForUser(currentUser.id);
  ...
  }, [isAdmin, currentUser, location.pathname, authIsLoading]);
  ...
  const isJessieInberg = currentUser?.username === ADMIN_USER_USERNAME;
    ...
    <nav className="bg-light-primary dark:bg-dark-primary shadow-md sticky top-0 z-40">
      <div className="container mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between h-16">
          ...
          <div className="flex items-center">
            {!isAdmin && currentUser && !isJessieInberg && ( // Hide for Jessie if not in admin, they have a special button
               <Button
                variant="primary"
                size="sm"
