...
const ProductForm: React.FC<ProductFormProps> = ({
  ...
  const [contactInfo, setContactInfo] = useState(initialProduct?.contactInfo || currentUser?.username || '');
  ...
  useEffect(() => {
    if (initialProduct) {
      ...
      setContactInfo(currentUser.username);
    }
  }, [initialProduct, currentUser]);
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    const productData = {
      ...
      userId: initialProduct?.userId || currentUser.id, 
      contactInfo,
      city,
      condition: condition as ProductCondition, // Cast because state allows '', but validation ensures it's set
      ...(initialProduct && { 
