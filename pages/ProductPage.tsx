...
const ProductPage: React.FC = () => {
  ...
  const fetchProductAndMessages = useCallback(async () => {
    ...
    try {
      ...
      if (fetchedProduct) {
        ...
        if (currentUser && fetchedProduct.userId !== currentUser.id) { 
           const fetchedMessages = await chatService.getMessagesForAd(id, currentUser.id);
           setMessages(fetchedMessages);
           await chatService.markMessagesAsRead(id, currentUser.id); 
        }
      } else {
  ...
  const handleSendMessage = async (e: FormEvent) => {
    e.preventDefault();
    if (!newMessageText.trim() || !product || !currentUser || product.userId === currentUser.id) return;
    setIsSendingMessage(true);
    try {
      const sentMessage = await chatService.sendMessage(product.id, currentUser.id, product.userId, newMessageText.trim());
  ...
  const isOwnAd = currentUser ? product.userId === currentUser.id : false;
            ...
            <div className="max-h-96 overflow-y-auto mb-4 p-3 bg-slate-50 dark:bg-slate-800 rounded-lg space-y-3">
              ...
              {messages.map(msg => (
                <div key={msg.id} className={`flex ${msg.senderId === currentUser.id ? 'justify-end' : 'justify-start'}`}>
                  <div className={`max-w-xs lg:max-w-md px-4 py-2 rounded-xl ${msg.senderId === currentUser.id ? 'bg-sky-500 text-white' : 'bg-slate-200 dark:bg-slate-700 text-light-text-primary dark:text-dark-text-primary'}`}>
                    <p className="text-sm">{msg.text}</p>
                    <p className={`text-xs mt-1 ${msg.senderId === currentUser.id ? 'text-sky-200 text-right' : 'text-slate-500 dark:text-slate-400 text-left'}`}>
