...
const MessagesPage: React.FC = () => {
  ...
  const fetchChatPreviews = useCallback(async () => {
    ...
    setError(null);
    try {
      const adIdsWithChats = await chatService.getChatsForUser(currentUser.id);
      ...
      for (const adId of adIdsWithChats) {
        ...
        const messagesForAd = await chatService.getMessagesForAd(adId, currentUser.id);
        const lastMessage = messagesForAd.length > 0 ? messagesForAd[messagesForAd.length - 1] : null;
        const unreadCount = messagesForAd.filter(msg => msg.receiverId === currentUser.id && !msg.read).length;
            ...
            >
              <div className="flex items-center space-x-4">
                ...
                <div className="flex-grow overflow-hidden">
                  ...
                  {preview.lastMessage && currentUser ? ( // Added currentUser check
                    <p className={`text-sm truncate ${preview.unreadCount > 0 ? 'font-bold text-sky-600 dark:text-dark-accent' : 'text-slate-500 dark:text-slate-400'}`}>
                      {preview.lastMessage.senderId === currentUser.id ? "Вы: " : ""}
