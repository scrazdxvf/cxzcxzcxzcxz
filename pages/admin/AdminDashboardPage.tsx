...
import { Product, AdStatus, User } from '../../types';
...
const AdminDashboardPage: React.FC = () => {
  const { users: allUsers, isLoading: authLoading } = useAuth(); // Get users from AuthContext
  ...
  const fetchDashboardData = useCallback(async () => {
    ...
    try {
      ...
      const usersRegisteredLast24h = allUsers.filter(user => user.createdAt >= twentyFourHoursAgo).length;
